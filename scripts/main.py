...
def process_chunk(chunk):
    ...
    for i in np.arange(0, 8, 2):
        ...
        lap = (chunk[:, i] + chunk[:, i + 1]) / 2 #TODO CHANGE DEPENDING ON # OF CHANNELS
        proc_lap = process_signal(lap, low=5, high=124, Q=30)
        #print(proc_lap.shape)
...
def get_threshold():
    """
    This thread reads the EMG LSL stream and creates a threshold for spacing
    """
    global emg_inlet
    global THRESHOLD


    print('Keep hand still. You will have to clench and relax your fist nontstop for 6 seconds.')
    print("***")
    time.sleep(1)
    ...
    data = []
    start = time.time()
    print('CLENCH AND RELAX YOUR FIST')
    # Read LSL stream
    while time.time()-start<6:
        sample, times = emg_inlet.pull_sample()
        data.append(np.mean(sample)) #TODO CHANGE DEPENDING ON # OF CHANNELS
    
    THRESHOLD = np.mean(np.array(data),axis=0)
